...
import { RefreshCw, ExternalLink, CheckCircle2, AlertCircle, Package } from "lucide-react";
import { syncBlingOrderAction } from "@/app/(admin)/admin/editora/pedidos/actions";
...
export function BlingSyncCard({ orderId, blingOrderId, initialSituacao }: Props) {
  ...
  return (
    <div className="bg-white rounded-xl border border-border p-5 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={handleSync}
          disabled={loading || !blingOrderId}
          title={!blingOrderId ? "Pedido ainda não enviado ao Bling" : "Sincronizar com Bling"}
      ...
      {blingOrderId ? (
        ...
        <div className="flex items-center gap-2 text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
          ...
          <span>Pedido ainda não registrado no Bling. Será enviado automaticamente no próximo pagamento aprovado.</span>
        </div>
      )}
    </div>
  );
